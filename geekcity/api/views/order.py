...
class OrderViewSet(APIView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        try:
            # 用户请求验证
            policy_course_list = request.data.get('policy_course_list')
            ...
            if balance > request.user.balance:
                raise Exception('账户中虚拟金额不足')
            # 检查用户提交的信息redis结算列表中是否存在，如果不存在，则需要用户从购物车中再次去结算
            payment_dict_bytes = redis_pool.conn.hget(settings.PAY_KEY,request.user.id)
            ...
            with transaction.atomic():
                ...
                self.ret.payment_type = payment_type
                # 生成支付宝URL地址
                if payment_type == 1:
